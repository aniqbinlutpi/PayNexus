...
class FraudDetectionService {
    async analyzeTransaction(transactionData) {
        try {
            logger_1.logger.info(`Analyzing transaction for fraud: ${transactionData.userId}`);
            const [userPattern, velocityAnalysis, deviceAnalysis, amountAnalysis, geographicAnalysis] = await Promise.all([
            ...
            logger_1.logger.error('Fraud detection analysis failed:', error);
            return {
                isFraudulent: false,
                riskScore: 0,
    ...
    async analyzeVelocity(userId, timestamp) {
        const oneHourAgo = new Date(timestamp.getTime() - 60 * 60 * 1000);
        const recentTransactions = await database_1.prisma.paymentTransaction.count({
            where: {
                userId,
                createdAt: {
                    gte: oneHourAgo
    ...
    async analyzeDevice(userId, deviceFingerprint) {
        if (!deviceFingerprint) {
            return { isNewDevice: true };
        }
        const existingSession = await database_1.prisma.userSession.findFirst({
            where: {
                userId,
                deviceInfo: {
                    path: ['fingerprint'],
    ...
    async analyzeGeographic(userId, ipAddress) {
        ...
        const recentSessions = await database_1.prisma.userSession.findMany({
            where: {
                userId,
                createdAt: {
    ...
    getMockLocationFromIP(ipAddress) {
        if (!ipAddress)
            return 'Unknown';
    ...
    async analyzeBehavioralPattern(userId, transactionData) {
        const reasons = [];
        let score = 0;
    ...
    calculateConfidence(pattern, transactionData) {
        let confidence = 50; // Base confidence
        // More historical data = higher confidence
        if (pattern.velocityPattern > 1)
            confidence += 20;
        if (pattern.frequentProviders.length > 2)
            confidence += 15;
        if (pattern.commonHours.length > 3)
            confidence += 15;
        return Math.min(confidence, 100);
    }
    async logFraudAnalysis(userId, analysis) {
...
FraudDetectionService.FRAUD_THRESHOLD = 80;
FraudDetectionService.HIGH_RISK_THRESHOLD = 60;
